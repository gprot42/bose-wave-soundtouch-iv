...
main() {
    ...
    if $MODE_SSH; then
        ...
        echo "  7. SSH in (no password):"
        echo "       ssh -o HostKeyAlgorithms=ssh-rsa -o PubkeyAcceptedAlgorithms=ssh-rsa -l root <speaker-ip>"
        echo "     Use the router/Ethernet IP, not 192.0.2.1 (SSH is not on the setup AP)."
        echo "  8. Make SSH persistent (required — survives reboots):"
        echo "       ./scripts/persist-ssh.sh <speaker-ip>"
        echo "     Or on-speaker: touch /mnt/nv/remote_services; mount -n -o remount,rw /; touch /etc/remote_services"
